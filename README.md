...
dotnet run -- convert-strokes input.mp4 output.lvfs --quality 82 --stroke-density 78 --surface-detail 65 --surface-opacity 10 --residual 6 --glow 38 --keyframe 30 --pipeline 4 --profile
dotnet run -- convert-strokes input.mp4 output.lvfs --vector-only --profile
...
- `--surface-opacity`: 0-100. Default is `10`. Keeps the raster-like fallback very faint so vector strokes and hatching carry the image instead of an opaque pixel grid.
- `--vector-only`: disables the surface and residual fallback layers so playback is only vector strokes/hatching on black.
...
```powershell
dotnet run -- convert-strokes input.mp4 preview.lvfs --stroke-density 45 --surface-detail 65 --surface-opacity 8 --residual 4 --max-frames 120 --profile
```
...
The GPU player supports `.lvfb` region files and `.lvfs` stroke-hybrid files. `.lvfb` playback tessellates polygons into triangles. `.lvfs` playback draws mostly-local surface subtiles, feathered residual patches, and capped thick/glowing stroke quads through OpenGL.
